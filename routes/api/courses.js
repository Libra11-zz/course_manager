...
router.get(
	...
	(req, res) => {
		...
		// 模糊查询
		if (keyword == undefined || keyword == '') {
			var findFiler = { yibaoming: { $in: [url.parse(req.url, true).query.pid] } }; // 检索全部
		} else {// 使用正则表达式的构造函数将字符串转为再正则对象
			const regexp = new RegExp(keyword, 'g');
			var findFiler = {
				$or: [
...
// 更改教师
router.post('/changecourse', passport.authenticate('jwt', { session: false }),
	(req, res) => {
		let result = {}
		Course.findOne(
			{ pid: req.body.pid }, (err, res2) => {
				result = res2
				const courseFields = {};
				if (req.body.pid) courseFields.pid = req.body.pid;
				if (req.body.name) courseFields.name = req.body.name;
...
router.delete(
	...
	// passport.authenticate('jwt', { session: false }),
	(req, res) => {
		Course.findOneAndRemove({ _id: url.parse(req.url, true).query._id }, (err, course) => {
			if (err) {
...
// 选课
router.post('/selectcou', passport.authenticate('jwt', { session: false }),
	(req, res) => {
		const updatefileds = {}
		const _id = req.body._id
		const pid = req.body.pid
		const length = req.body.length
		Course.update({ "_id": _id },
			{ '$addToSet': { yibaoming: pid } }, (err, data) => {
				if (err) {
					console.log(err);
